...
export const mapToFavoriteItem = (product: Product | ItemCard): FavoriteItem => {
  const id = typeof product.id === "number"
    ? (product as Product).itemId
    : product.id;
  ...
  return {
    ...
    fullPrice: "fullPrice" in product ? product.fullPrice : product.priceRegular,
