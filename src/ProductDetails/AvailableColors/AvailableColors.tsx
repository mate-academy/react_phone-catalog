...
export const AvailableColors: React.FC<Props> = ({ colors, currentColor }) => {
  ...
  return (
    <div className={style['available-colors']}>
      <p className={style['available-colors__title']}>Available colors</p>
      <ul className={style['available-colors__list']}>
        {colors.map(color => {
          ...
          return (
            <li
              className={classNames(style['available-colors__item'], {
                [style['available-colors__item--active']]: isActive,
              })}
              key={color}
            >
              <Link
                style={{ backgroundColor: colorMap[color.replaceAll(' ', '')] }}
                className={style['available-colors__link']}
                to={isActive ? '' : newPath}
              ></Link>
            </li>
          );
