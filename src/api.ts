  ...
  return getProducts()
    .then(products => products
      ...
      .sort((productFirst, productSecond) => (
        (productSecond.fullPrice - productSecond.price)
...
export const getProductsFilter = (
  ...
  if (query) {
    productsFilter = productsFilter.filter(
      product => normalizeText(product.name).includes(normalizeText(query)),
    );
  }
  ...
  switch (sortBy) {
    ...
    case 'name':
      return productsFilter.sort(
        (good1, good2) => normalizeText(good1.name).localeCompare(
          normalizeText(good2.name),
        ),
      );
    case 'price':
