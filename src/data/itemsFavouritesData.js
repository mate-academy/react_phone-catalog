...
export const itemsFavoritesData = [
  {
...
export const addFavorite = product => {
  const exists = itemsFavoritesData.find(
    fav => fav.titleProduct === product.titleProduct,
  );

  if (!exists) {
    itemsFavoritesData.push(product);
    window.dispatchEvent(new Event('favoritesUpdated'));
  }
};

export const removeFavorites = name => {
  const indexIsReal = itemsFavoritesData.findIndex(
    fav => fav.titleProduct === name,
  );

  if (indexIsReal !== -1) {
    itemsFavoritesData.splice(indexIsReal, 1);
    window.dispatchEvent(new Event('favoritesUpdated'));
  }
};
