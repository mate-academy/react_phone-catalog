...
const ProductDetails = () => {
  ...
  const [productFull, setProductFull] = useState<IProductDetails | undefined>(undefined);
  const [product, setProduct] = useState<IProductCard | undefined>(undefined);
  const [isSelectedImg, setIsSelectedImg] = useState<string | undefined>(undefined);
  const [isSelectedColor, setIsSelectedColor] = useState<string | undefined>(undefined);
  const [isSelectedCapacity, setIsSelectedCapacity] = useState<string | undefined>(undefined);
    ...
    const fetchData = async() => {
      ...
      try {
        if (productId && category) {
          ...
          const productFull = await ProductService.getByProductIdDetails(productId, category);
          const product = await ProductService.getByProductId(productId);
          setProductFull(productFull);
          setProduct(product);
          setIsSelectedImg(productFull?.images[0]);
          setIsSelectedColor(productFull?.color);
          setIsSelectedCapacity(productFull?.capacity);
        }
      } catch {
        console.error('Product not found component');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className={styles.container}>
      {isLoading ? (
        ...
        productFull ? (
          <>
            <Breadcrumbs categoryName={category} productName={productFull?.name} />
    
            <Link to={`/${category}`} className={styles.backLink}>
              <div className={styles.back}>
                <img src="/images/icons/VectorLeft.png" className={styles.back__img}/>
                <h4>Back</h4>
              </div>
            </Link>
              ...
              <div className={styles.images}>
                <div className={styles.images__small}>
                  {productFull?.images.map(image => (
                    <div className={cn(styles.images__smallBlock, {
                      [styles.images__smallBlock__selected]: isSelectedImg === image,
                    })}>
                    ...
                    {productFull.colorsAvailable.map(color => (
                      <div 
                        className={cn(styles.details__colorsPallet, {
                          [styles.details__colorsPallet__selected]: isSelectedColor === color,
                        })}
                ...
                <div className={styles.details__capacity}>
                  ...
                  <div className={styles.details__capacityBlock}>
                    {productFull.capacityAvailable.map(capacity => (
                      <div className={cn(styles.details__option, {
                          [styles.details__option__selected]: isSelectedCapacity === capacity,
                        })}
            ...
            <EmptyContent img='/img/product-not-found.png' title='Product not found'/>
          )
      )}
    </div>
  )
