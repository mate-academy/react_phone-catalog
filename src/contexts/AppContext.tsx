import { useState, createContext, ReactNode, useMemo, useEffect, useContext } from 'react';
import { getCartProducts, getFavouriteProducts, saveCartProducts, saveFavouriteProducts } from '../modules/shared/services/localStorage';
...
type AppContextType = {
  ...
  setFavouriteProductsIds: (prev: number[]) => void;
  setCartProductsIds: (prev: number[]) => void;
...
export const AppProvider: React.FC<Props> = ({ children }) => {
  const [favouriteProductsIds, setFavouriteProductsIds] = useState<number[]>(getFavouriteProducts());
  const [cartProductsIds, setCartProductsIds] = useState<number[]>(getCartProducts());
  ...
  function toggleFavouriteCard(cardId: number) {
    if (favouriteProductsIds.includes(cardId)) {
      const updatedFavourites = favouriteProductsIds.filter((id: number) => id !== cardId);
