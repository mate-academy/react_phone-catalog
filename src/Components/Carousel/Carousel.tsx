...
type Props = {
  products: Product[],
  title: string,
  // setOrderedProducts: (value: any) => void,
  // orderedProducts: Product[],
  // searchParams: URLSearchParams,
};

export const Carousel: React.FC<Props> = ({ products, title /* setOrderedProducts, orderedProducts */ }) => {
  ...
  const onNext = () => {
    setOffset((prev) => (
      Math.min(prev += slideWidth, max)
    ));
  };

  const onPrev = () => {
    setOffset((prev) => (
      Math.max(prev -= slideWidth, 0)
    ));
  };
    ...
    <section className="carousel">
      <div className="carousel__top">
        ...
        <div className="carousel__buttons">
          ...
          >
            <img src="Images/arrow-icon--left.svg" alt="Arrow icon right" style={{ transform: 'rotate(180deg)' }} />
          </button>
        </div>
      </div>
