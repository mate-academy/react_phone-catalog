...
import { getPoductsByCategory, getProductById, getProducts } from '../../Helpers/Helpers';
...
export const ProductPage: React.FC<Props> = ({
  ...
  const loadProductsData = async () => {
    try {
      ...
      setPhonesProducts(phonesData.filter((el) => el.name.toLowerCase().includes(query.toLowerCase())));
    } catch (error) {
      setIsError(true);
      setIsErrorMessage(
        'Unfortunately, an error occurred, it is not possible to download data from the server. Try again later, we will resolve this issue soon. Thank you',
      );
    } finally {
      setIsLoader(false);
    }
  ...
  const loadDataProduct = async () => {
    try {
      ...
      setIsError(true);
      setIsErrorMessage('Unfortunately, an error occurred, it is not possible to download data from the server. Try again later, we will resolve this issue soon. Thank you');
    } finally {
      setIsRender(false);
    }
  };
            ...
            <div className="products__wrapper">
              <div className="container">
                {isError ? (
                  ...
                  <>
                    <Location />
                    <Placeholder title={title} productsCount={products.length} />
