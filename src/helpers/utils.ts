...
export const useLocalStorage = <T>(
  ...
  const valueFromStorage: T[] = JSON.parse(localStorage.getItem(key) || '[]');
...
export const getItemSaverFunction = (
  ...
  return (newProduct: Product) => {
    ...
    const savedProduct = savedProducts.find(item => item.id === newProduct.id);
    const savedProductIndex = savedProduct
      ? savedProducts.indexOf(savedProduct)
      : -1;
    const isChangingQuantity =
      savedProduct?.quantity !== newProduct.quantity && newProduct?.quantity;
    ...
    if (savedProduct) {
      newProducts = copyOfProducts.filter(product => {
      ...
      const useProduct =
        key === 'cart' ? { ...newProduct, quantity: 1 } : { ...newProduct };

      newProducts = [...savedProducts, useProduct];
    }
