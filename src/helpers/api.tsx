...
export const getProducts = () => {
  return fetch(`${BASE_URL}.json`, { method: 'GET' }).then(response => {
    if (!response.ok) {
      throw new Error();
    }
...
export const getHotPriceProducts = () =>
  getProducts().then(res =>
    res.sort((good1: Product, good2: Product) => {
...
export async function getProductDetails(productId: string | undefined = '') {
  const response = await wait().then(() =>
    fetch(`${BASE_URL}/${productId}.json`, { method: 'GET' }),
  );
