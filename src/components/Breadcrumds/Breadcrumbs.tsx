...
export const Breadcrumbs: React.FC<Props> = ({ title, paths, details }) => {
  const theme = useAppSelector(state => state.themeSwitcher.theme);
  const homeImg = theme === ThemeVars.DARK ? Home_dark : Home;
  const vectorRight =
    theme === ThemeVars.DARK ? Vec_light_right_dark : Vec_light_right;
  const productsTitle = `productsPage__phones theme-${theme}`;
  const pathClass = `breadcrumbs__pathsLink themeDetails-${theme}`;
      ...
      {!!details && (
        <>
          <NavLink to={`/${paths}`} className={pathClass}>
            <div className={breadCrumbsPaths}>{paths}</div>
          </NavLink>
          <img
            src={vectorRight}
            alt="Vector_light_right"
