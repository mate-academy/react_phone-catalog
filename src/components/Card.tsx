...
export const Card: React.FC<Props> = ({ products }) => {
  ...
  const isRightPath = PATHS_WHERE_NEED_FIND.some(path => pathname.slice(1) === path) || pathname.slice(1) === 'favourites';

  return (
    <div className={`flex gap-x-[16px] ${isRightPath ? 'grid grid-cols-[repeat(auto-fit,minmax(229px,1fr))] gap-y-[40px]' : ''}`}>
      {products && products.map(product => (
        ...
        >
          <Link
            to={PATHS_WHERE_NEED_FIND.some(path => path === pathname.slice(1))
              ? `${product.itemId}`
