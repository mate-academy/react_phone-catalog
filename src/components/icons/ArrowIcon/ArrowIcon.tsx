/* eslint-disable max-len */
import React from 'react';
import classNames from 'classnames';
import styles from './ArrowIcon.module.scss';

type Direction = 'left' | 'right' | 'up' | 'down';

interface ArrowIconProps {
  color?: string; // Колір, наприклад, 'currentColor', '#0F0F11', 'gray'
  direction?: Direction; // Напрямок (для обертання)
  className?: string; // Додатковий клас для зовнішньої обгортки (якщо потрібно)
}

export const ArrowIcon: React.FC<ArrowIconProps> = ({
  color = 'currentColor', // Використовуємо 'currentColor' за замовчуванням
  direction = 'right',
  className,
}) => {
  const svgContent = (
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M5.52876 3.52861C5.78911 3.26826 6.21122 3.26826 6.47157 3.52861L10.4716 7.52861C10.7319 7.78896 10.7319 8.21107 10.4716 8.47141L6.47157 12.4714C6.21122 12.7318 5.78911 12.7318 5.52876 12.4714C5.26841 12.2111 5.26841 11.789 5.52876 11.5286L9.05735 8.00001L5.52876 4.47141C5.26841 4.21107 5.26841 3.78896 5.52876 3.52861Z"
      fill={color}
    />
  );

  return (
    <svg
      className={classNames(
        styles.arrowIcon,
        styles[`arrowIcon--${direction}`],
        className,
      )}
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true" // Для доступності
    >
      {svgContent}
    </svg>
  );
};
