...
export const Arrays = () => {
  ...
  const handleNextImg = () => {
    if (theme === ThemeVars.DARK) {
      ...
      return currentPage === lastPage ? `${Vec_light_right}` : `${Arrow_Right}`;
    }
  };
  ...
  return (
    <div className="arrays">
      ...
      >
        <img src={handleImgPrev()} className="arrays__img" alt={Arrow_Left} />
      </button>
      <button
      ...
      >
        <img src={handleNextImg()} className="arrays__img" alt={Arrow_Right} />
      </button>
    </div>
  );
};
