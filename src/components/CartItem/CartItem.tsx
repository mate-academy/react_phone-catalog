...
export const CartItem: React.FC<Props> = ({ item }) => {
  ...
  const hasDiscount = product.fullPrice !== undefined && product.fullPrice > product.price;
  ...
  return (
    <li className={styles.cartItemWrap}>
      <article className={styles.cartItem} aria-labelledby={`ci-title-${product.itemId}`}>
        <div className={styles.cartItem__left}>
          ...
          <div className={styles.cartItem__info}>
            <h2 id={`ci-title-${product.itemId}`} className={styles.cartItem__title}>
              {product.name}
            </h2>
          </div>
        </div>
        ...
        <div className={styles.cartItem__controls}>
          ...
          <div className={styles.cartItem__price}>
            {hasDiscount && <div className={styles.cartItem__old}>{formattedOldPrice}</div>}
