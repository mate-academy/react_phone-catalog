...
export const PhonesPage: React.FC = () => {
  ...
  const perPageNumber =
    perPage === 'all' ? sortedPhones.length : Number(perPage ?? 16);

  const start = (currentPageNumber - 1) * perPageNumber;
  const end = start + perPageNumber;

  const visibleProducts = sortedPhones.slice(start, end);

  const totalPages = Math.ceil(sortedPhones.length / perPageNumber);
    ...
    <main>
      <div className="elements">
        <div className="elements__wrapper">
          ...
          <div className="elements__nav-text small-text">Phones</div>
        </div>

        <h1 className="elements__title">Mobile Phones</h1>
