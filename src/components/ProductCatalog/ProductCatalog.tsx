...
export const ProductCatalog: React.FC<Props> = ({ products }) => {
  ...
  const itemsOnPage = Number(searchParams.get('itemsOnPage')) || 4;
  const currentPage = Number(searchParams.get('currentPage')) || 1;

  const handleSortChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setSearchParams(
      getSearchWith({
        sortOption: event.target.value === 'Newest' ? null : event.target.value,
        currentPage: null
      },
  ...
  const handleItemsOnPage = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setSearchParams(
      getSearchWith({
        itemsOnPage: event.target.value === '4' ? null : event.target.value,
        currentPage: null
      },
      // eslint-disable-next-line @typescript-eslint/indent
        searchParams),
  ...
  const totalPagesArray = Array.from(
    { length: totalPages }, (_, index) => index);
    ...
    <div className="catalog">
      <div className="catalog__controls">
        ...
        <div className="catalog__controls__box">
          ...
          >
            <option className='catalog__dropdown__options' value={4}>4</option>
            <option className='catalog__dropdown__options' value={8}>8</option>
            <option
              className='catalog__dropdown__options'
              value={16}>16</option>
            <option
