...
export const TabletsPage: React.FC = () => {
  ...
  const perPageNumber =
    perPage === 'all' ? sortedTablets.length : Number(perPage ?? 16);

  const start = (currentPageNumber - 1) * perPageNumber;
  const end = start + perPageNumber;

  const visibleProducts = sortedTablets.slice(start, end);

  const totalPages = Math.ceil(sortedTablets.length / perPageNumber);
    ...
    <main>
      <div className="elements">
        <div className="elements__wrapper">
          ...
          <div className="elements__nav-text small-text">Tablets</div>
        </div>

        <h1 className="elements__title">Tablets</h1>
