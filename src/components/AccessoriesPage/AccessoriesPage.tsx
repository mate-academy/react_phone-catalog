...
import { СardAccessories } from '../../cardAccessories';
...
export const AccessoriesPage = ({
  ...
  const [gadgets, setGadgets] = useState(accessories);
  ...
  useEffect(() => {
    const sortedPhones = [...accessories];
  ...
  const totalPages =
    elements === 'all' ? 1 : Math.ceil(accessories.length / +elements);
  ...
  const end = elements === 'all' ? accessories.length : start + +elements;
  const visibleCards = gadgets.slice(start, end);

  const convertToCardPhone = (accessory: СardAccessories): CardPhone => {
    return {
      id: accessory.id,
      ...
      <div className="availability">
        <main className="functional">
          <section className="navigate">
            ...
            <p className="navigate__text">Accessories</p>
          </section>

          <h1 className="functional__text">Accessories</h1>
          <p className="functional__models">34 models</p>
            ...
            <div className="cardsContainer">
              {visibleCards.map(item => {
                ...
                const isInCart = cartItems.some(
                  phone => phone.id === convertToCardPhone(item).id,
                );
                    ...
                    <div className="card__buttons">
                      <button
                        className={cn(
                          ...
                          { 'card__buttons--active': isInCart },
                        )}
                        onClick={() => addCartItems(convertToCardPhone(item))}
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          addFavouritesItems(convertToCardPhone(item))
                        }
                      >
                        <div
