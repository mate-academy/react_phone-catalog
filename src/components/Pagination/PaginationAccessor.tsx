...
export const PaginationAccessor = () => {
  const { accessories } = useContext(CatalogContext);
  ...
  const itemsPerPage = perPage === 'all' ? accessories.length : perPage;

  const total = () => {
    if (perPage === 'all') {
      setSearchParams({
        page: `${1}`.toString(),
        perPage: `${accessories.length}`.toString(),
      });
