...
export const CartCard: React.FC<CartCardProps> = ({ cartCardData }) => {
  const { name, image, itemId } = cartCardData;
  ...
  const correctCard = cart.cartItems.find(
  ...
  return (
    <div className="cartProductCard">
      ...
      <p className="cartProductCard__name">{name}</p>

      <div className="cartProductCard__btnContainer">
        <button
          type="button"
          className={classNames('cartProductCard__btnContainer__btn', {
            'cartProductCard__btnContainer__btn--disabled': productCount === 1,
          })}
        ...
        <p className="cartProductCard__btnContainer__number">{productCount}</p>
        <button
          type="button"
      ...
      <h2 className="cartProductCard__price">{`$${totalPrice}`}</h2>
    </div>
  );
};
