...
export const CategoryCart: React.FC<Props> = ({ category }) => {
  const { phonesFromServer, tabletsFromServer, accessoriesFromServer } = useContext(CatalogContext);
  ...
  return (
    <Link to={`/${category}`} className="categories__item">
      <div className={`categories__image-container categories__image-container--${category}`}>
        <img
