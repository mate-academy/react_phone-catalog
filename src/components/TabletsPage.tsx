...
export const TabletsPage = React.memo(() => {
  ...
  switch (filter) {
    case '':
      sortedProducts = tablets;
      activeFilter = 'All';
      break;

    case 'alphabetically':
      sortedProducts = tablets.sort((a, b) => a.name.localeCompare(b.name));
      activeFilter = 'Alphabetically';
      break;

    case 'newest':
      sortedProducts = tablets.sort((a, b) => a.age - b.age);
      activeFilter = 'Newest';
      break;

    case 'oldest':
      sortedProducts = tablets.sort((a, b) => a.price - b.price);
      activeFilter = 'Oldest';
      break;

    case 'asc':
      sortedProducts = tablets.sort((a, b) => getFullPrice(+a.price, +a.discount) - getFullPrice(+b.price, b.discount));
      activeFilter = 'Price low to high';
      break;

    case 'desc':
      sortedProducts = tablets.sort((a, b) => getFullPrice(+b.price, +b.discount) - getFullPrice(+a.price, a.discount));
  ...
  return (
    <>
      <div className="container grid">
        ...
        <h1 className="subheader__title grid__item--1-7">Tablets</h1>

        <p className="subheader__quantity grid__item--1-2 body-text">{`${tablets.length} models`}</p>

        <p className="sort__heading grid__item--1-2 small-text">Sort by</p>

        <button
          type="button"
          className="sort button-text grid__item--1-4"
          onClick={() => {
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="sort__option body-text"
              onClick={() => {
                setItemsPerPage(tablets.length);
                setCurrentPage(1);
              }}
            >
              All
