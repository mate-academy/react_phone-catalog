...
export const Cart = () => {
  ...
  const totalPrice = cart.reduce(
    (sum, product) =>
      sum + (product.priceRegular || 0) * (product.quantity || 1),
    0,
  );
  ...
  return (
    <div className="font-mont px-4 sm:px-6 md:px-6 lg-1.5:px-8 xl:px-[152px]">
      <Link to={'/phones'} className="flex items-center pt-10 group">
        ...
        />
        <p className="text-text-color-base-white text-xs font-bold group-hover:text-color-btn-purple">
          Back
        </p>
      </Link>
      ...
      {cart_num === 0 ? (
        <p className="pt-48 text-text-color-base-white text-5xl text-center uppercase font-semibold">
          Your cart is empty
        </p>
      ) : (
        <div className="pt-8 flex flex-col lg-1.5:flex-row gap-4">
          ...
          <div
            className="w-full max-w-[752px] text-sm leading-[21px] border border-color-border flex flex-col items-center px-4 sm:px-6 py-6
          ...
          >
            ...
            <span className="font-semibold text-text-color-base-grey">{`Total for ${cart_num} items`}</span>
            <div className="mt-6 block max-w-[320px] w-full h-[1px] bg-color-border">
              &nbsp;
            </div>
            <button
