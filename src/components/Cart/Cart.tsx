...
export const Cart:React.FC = () => {
  ...
  return (
    ...
    <section className="flex gap-4 max-w-[1136px] mx-auto mt-10">
      {gadgets.length ? (
        <>
          <div className="flex flex-col gap-4 flex-grow-[2]">
            {gadgets.map(gadget => <CartItem item={gadget} setPrice={setPrice} key={gadget.id} />)}
          </div>
          <div className="flex-col flex-grow-[1] items-center justify-center border border-elements p-6 h-min">
            ...
            <p className="text-center text-[14px] font-semibold text-secondary">{`Total for ${gadgets.length} item${gadgets.length === 1 ? '' : 's'}`}</p>
            <div className="h-[1px] w-full bg-elements my-6" />
            <button
              type="button"
              className="text-white h4 bg-primary w-full py-[14px] shadow"
      ...
      ) : <p className="h1 text-altRed flex justify-center mx-auto">Cart is empty</p>}
