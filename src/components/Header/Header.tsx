...
const getClassLinkButton = ({ isActive }: { isActive: boolean }) => classNames(
  'nav-box__button', { 'is-active': isActive },
);
const getClassLinkButtonMenu
  = ({ isActive }: { isActive: boolean }) => classNames(
    'nav-box__button nav-box__button--menu', { 'is-active': isActive },
  );
                    ...
                    orderedProducts.length > 0 && (
                      <span className="nav-box__counter icon--counter">
                        {orderedProducts.reduce(
                          (sum, value) => sum + value.quantity, 0,
                        )}
                      </span>
                    )
                  }
