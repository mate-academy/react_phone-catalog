...
const CartItem: React.FC<Props> = ({ product }) => {
  ...
  return (
    <div className="border justify-between gap-4 flex-col font-mont border-gray-200 px-6 py-4  sm:gap-6 flex sm:flex-row items-center h-40 sm:h-32 sm:w-[752px]">
      <div className="flex flex-row justify-between gap-4 items-center w-full">
        <button onClick={() => handleDeleteProduct(product.id)}>
          <img
            src="../../../public/img/icons/X button.svg"
            alt="remove button"
          />
        </button>
      ...
      <div className="flex flex-row gap-4 justify-between items-center w-full sm:w-fit">
        <div className="flex flex-row items-center h-8 w-24 justify-around gap">
          <button
