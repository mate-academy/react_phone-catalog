...
const Pagination: React.FC<Props> = ({ currentPage, totalPages, onPageChange }) => {
  ...
  const renderPageNumbers = () => {
    ...
    if (startPage > 1) {
      ...
      if (startPage > 2) {
        pageNumbers.push(<span key="start-ellipsis" className="flex justify-center items-center size-8">...</span>);
      }
    }
    ...
    if (endPage < totalPages) {
      if (endPage < totalPages - 1) {
        pageNumbers.push(<span key="end-ellipsis" className="flex justify-center items-center size-8">...</span>);
      }
      pageNumbers.push(
        <button
          key={totalPages}
