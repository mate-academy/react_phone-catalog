...
export const CartPage: React.FC = () => {
  ...
  const [activeButtons, setActiveButtons] = useState<{
    [key: number]: { increase: boolean; decrease: boolean };
  }>({});
            ...
            {cart.map(item => (
              <div className={styles.itemCart} key={item.id}>
                <div className={styles.informationItem}>
                  ...
                  />
                  <h3 className={styles.productCart__title}>
                    {item.product.name}
                  </h3>
                </div>
                <div className={styles.quantity}>
                  <div
                    className={`${styles.productCart__quantity} ${styles.productQuantity}`}
                  >
                    <button
                      className={
                    ...
                    <button
                      ...
                      onClick={() => handleButtonPress(item, true)}
                    >
                      +
                    </button>
                  </div>
