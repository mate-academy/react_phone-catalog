...
const CHECKOUT_MESSAGE = 'Checkout is not implemented yet. Do you want to clear the Cart?';
      ...
      <div className={styles.cartPage__content}>
        <div className={styles.cartPage__list}>
          <ul className={styles.cartPage__items}>
            {cartItems.length > 0 ? (
              cartItems.map(item => <CartItem key={item.product.itemId} item={item} />)
            ) : (
        ...
        <aside className={styles.summary}>
          <div className={styles.summary__inner}>
            ...
            <button className={styles.summary__checkout} onClick={handleCheckoutClick}>
              Checkout
            </button>
          </div>
        </aside>
