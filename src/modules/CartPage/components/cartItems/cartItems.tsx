    ...
    {products.map(product =>
      <div key={product.id } className={styles.cartItem}>
        <div className={styles.cartItem__info}>
          <IoCloseSharp className={classNames(styles.cartItem__icon, [styles['cartItem__icon--close']])}
      onClick={()=>handleItemClear(product.id)}    />
          <Link to={ `/${product.category}/${product.id}`} className={styles.cartItem__link} >
          <img src={`./${product.images[0]}`} className={styles.cartItem__image} />
          <p className={styles.cartItem__title}>{product.name}</p></Link>
        </div>
        <div className={styles.cartItem__quantity}>
          <div className={styles.cartItem__box}>
            <FiMinus className={classNames(styles.cartItem__icon, { [styles['cartItem__icon--disabled']]: product.quantity === 1})}
