...
export function useProductsForCategory(opts: UseProductsParams): UseProductsResult {
  const {
    type,
    syncWithUrl = true,
    initialQuery = '',
  ...
  const query = syncWithUrl ? (searchParams.get('query') ?? initialQuery) : initialQuery;
  const sort = syncWithUrl ? ((searchParams.get('sort') as SortKey) ?? initialSort) : initialSort;
  const page = syncWithUrl
    ? parseInt(searchParams.get('page') ?? '', 10) || initialPage
    : initialPage;
  const perPage = syncWithUrl ? (searchParams.get('perPage') ?? initialPerPage) : initialPerPage;
  ...
  const perPageNum = perPage === 'all' ? Infinity : parseInt(perPage, 10) || Infinity;
  const totalPages = perPageNum === Infinity ? 1 : Math.max(1, Math.ceil(totalItems / perPageNum));
