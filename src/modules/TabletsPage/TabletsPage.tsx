...
export const TabletsPage = () => {
  const tabletList = tablets;
  const DEFAULT_PER_PAGE = 16;
  const DEFAULT_PAGE = 1;

  let filteredTabletList = [...tabletList];
  ...
  switch (currentFilter) {
    case 'age':
      filteredTabletList = [...tabletList].reverse();
      break;
    case 'title':
      filteredTabletList.sort((a: Product, b: Product) =>
        a.name.localeCompare(b.name),
      );
      break;
    case 'price':
      filteredTabletList.sort(
        (a: Product, b: Product) =>
  ...
  const endItem = Math.min(startItem + perPage - 1, tabletList.length);
