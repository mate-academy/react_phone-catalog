...
import { shufflePtoducts } from '../../utils/shuffleProduct';
...
export const ProductDetailsPage = () => {
  ...
  const [productsInCategory, setProductsInCategory] = useState<ProductDetailed[]>([]);
  ...
  const getSuggestedProducts = (products: ProductDetailed[], productId: string) => {
    const suggest = products.filter(product => product.id !== productId);

    return shufflePtoducts(suggest).slice(0, 10);
  };

  useEffect(() => {
    if (!productsInCategory || !productId) return;
  ...
  const updateProduct = (newColor: string, newCapacity: string) => {
    ...
    const newProductId = getNewProductId(product?.namespaceId, newColor, newCapacity);
  ...
  return (
    <div className={styles.productDetailsPage}>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.details}>
              ...
              {product.colorsAvailable.map(c => (
                ...
                >
                  <div
                    ...
                    style={{ backgroundColor: colorOptions[c.toLowerCase()] || c }}
                  ></div>
                </div>
              ))}
            </div>
          ...
          <div className={styles.pricesContainer}>
            <p className={styles.pricesContainer__priceDiscount}>${product.priceDiscount}</p>
            <p className={styles.pricesContainer__priceRegular}>${product.priceRegular}</p>
          </div>
          ...
          <div className={styles.specsContainer}>
            ...
            <div className={styles.specsContainer__item}>
              ...
              <p className={styles.specsContainer__value}>{product.resolution}</p>
            </div>
            <div className={styles.specsContainer__item}>
              <p className={styles.specsContainer__name}>Processor</p>
              <p className={styles.specsContainer__value}>{product.processor}</p>
            </div>
            <div className={styles.specsContainer__item}>
        ...
        <div className={styles.techSpecs}>
          ...
          <div className={styles.techSpecs__container}>
            ...
            <div className={styles.techSpecs__item}>
              ...
              <p className={styles.techSpecs__value}>{product.cell.join(', ')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.suggestedProducts}>
        <BrandModelsSlider title="You may also like" products={suggestedProducts} />
      </div>
    </div>
  );
};
