...
export const AccessoriesPage: React.FC = () => {
  ...
  const sortedAccessories = useMemo(() => {
    ...
    data = data.filter(product => product.category === 'accessories');
  ...
  const totalPages = useMemo(() => {
    ...
    return Math.ceil(sortedAccessories.length / parseInt(perPage));
  }, [sortedAccessories, perPage]);
  ...
  const visibleAccessories = useMemo(() => {
    ...
    return sortedAccessories.slice(startIndex, endIndex);
  ...
  return (
    <div className={`${styles.accessoriesPage} ${styles.container}`}>
      {isLoading ? (
        <Loader />
      ) : (
        <>
          <nav className={styles.breadcrumbs} aria-label="breadcrumb">
            ...
            <span className={styles.breadcrumbsCurrent}>Accessories</span>
          </nav>

          <h1 className={styles.accessoriesPageTitle}>Accessories</h1>
          <p className={styles.accessoriesPageCount}>
            {sortedAccessories.length} models
          </p>

          <div className={styles.accessoriesPageFilters}>
          ...
          <div className={styles.accessoriesPageGrid}>
            {visibleAccessories.length > 0 ? (
              ...
              <p className={styles.accessoriesPageEmpty}>
