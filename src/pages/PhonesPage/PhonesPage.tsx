    ...
    if (phonesFromServer && page && perPage) {
      ...
      switch (sort) {
        case SortParametr.NAME:
          sortedProducts = sortedProducts.sort((a, b) => a.name.localeCompare(b.name)).slice(startItem, endItem)
          break;
        case SortParametr.PRICE:
          sortedProducts = sortedProducts.sort((a, b) => a.priceRegular - b.priceRegular).slice(startItem, endItem)
          break;
  ...
  return (
    <section className="phones first-screen">
      <div className="container">
        ...
        <Outlet />
        {(phonesForShow.length > 0 && perPage &&phonesForShow.length > +perPage) && <Pagination
          itemsNumber={phonesForShow.length}
        />}
      </div>
    </section>
  )
