...
export const HomePage = () => {
  ...
  const handleBannerScrolling = useCallback(() => {
  ...
  const handleBannerSwipe = useCallback(
    (side: 'right' | 'left') => {
      setCurrentPic(prev => {
        if (side === 'right') {
          return prev + 1;
  ...
  const handleInterval = useCallback(() => {
    ...
    intervalIdRef.current = setInterval(handleBannerScrolling, 5000);
  }, [handleBannerScrolling]);
  ...
  }, [handleBannerSwipe, handleInterval, intervalIdRef]);
