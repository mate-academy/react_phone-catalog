...
export const AccessoriesPage = () => {
  ...
  const lastItem = Math.min(+itemsOnPage * +currentPage, accessories.length);

  useEffect(() => {
    setError(false);
    setIsLoading(true);

    getProductsByCategory(Categories.Accessories)
      .then(setAccessories)
      .catch(() => setError(true))
  ...
  const visibleItems =
    itemsOnPage === 'all'
      ? filteredAccessories
      : filteredAccessories.slice(firstItem - 1, lastItem);
    ...
    <div className="product">
      <div className="container">
        <div className="product__content">
          ...
          <h1 className="product__title title--h1">Accessories</h1>
          {!isLoading && (
            <p className="product__amount">{`${accessories.length} models`}</p>
          )}
