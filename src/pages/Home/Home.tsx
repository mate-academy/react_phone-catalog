...
function Home() {
  const banners = [
  { id: 1, desktop: './img/Banner.svg', mobile: './img/banner-mobile.svg' },
  { id: 2, desktop: './img/banner-accessories.png', mobile: './img/banner-accessories.svg' }, // замініть на ваші шляхи
  { id: 3, desktop: './img/home_banner1.png', mobile: './img/banner-phones.svg' },
];
...
useEffect(() => {
  ...
  const handleNext = () => setStartIndex(prev => Math.min(prev + 1, sortedProducts.length - visibleCount));
  ...
  const handleHotNext = () => setHotStartIndex(prev => Math.min(prev + 1, discountProducts.length - visibleCount));
