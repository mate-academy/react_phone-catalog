import { useNavigate } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { useLocalStorage } from 'usehooks-ts';
import { BasketGoods, Product } from '../types/product';
import { getProductsById } from '../api/products';
import { Loader } from '../components/Loader';
import { BasketCard } from '../components/BasketCard';
import arrowIcon from '../images/icons/arrow-icon-disable.svg';
import { ButtonCard } from '../components/ButtonCard';

export const BasketPage = () => {
  const navigate = useNavigate();
  const [basketGoods, setBasketGoods] = useLocalStorage<BasketGoods[]>(
    'basketGoods',
    [],
  );
  const sumOfQuantity = basketGoods.reduce(
    (acc, item) => acc + item.quantity,
    0,
  );

  const { isLoading: isBasketProducts, data: basketProducts } = useQuery({
    queryKey: ['basketProducts', basketGoods.length],
    queryFn: () => getProductsById(basketGoods.map(item => item.id)),
  });

  // const totalSumGoods =
  //   basketProducts?.length &&
  //   basketGoods.reduce(
  //     (acc, item) =>
  //       acc +
  //       item.quantity *
  //         basketProducts[
  //           basketProducts?.findIndex(product => product.itemId === item.id) ||
  //             0
  //         ].price,
  //     0,
  //   );
  // console.log(basketProducts);

  const handleChangeQuantity = (id: Product['itemId'], quantity: number) => {
    setBasketGoods(c => {
      const newBasket = [...c];

      // newBasket[newBasket.findIndex(item => item.id === id)].quantity =
      //   quantity;
      const test = newBasket.map(item =>
        item.id === id ? { ...item, quantity } : item,
      );

      return test;
    });
  };

  return (
    <main className="content pb-14 pt-6 md:pb-16 lg:pb-20">
      <section>
        <div className="mt-6 flex items-center gap-1 md:mt-10">
          <img
            src={arrowIcon}
            alt="Arrow Back"
            className="h-4 min-w-4 -rotate-90"
          />
          <button onClick={() => navigate(-1)}>
            <small className="font-bold text-secondary">Back</small>
          </button>
        </div>

        <h1 className="mt-6 md:mt-4">Cart</h1>

        <div className="mt-8 grid-cols-12 gap-4 lg:grid">
          {isBasketProducts ? (
            <Loader />
          ) : (
            <div className="flex flex-col gap-4 lg:col-span-8">
              {basketProducts?.map((product, index) => {
                // console.log(basketGoods[index].id);

                return (
                  <BasketCard
                    quantity={basketGoods[index].quantity || 1}
                    onChange={handleChangeQuantity}
                    key={product.id}
                    product={product}
                  />
                );
              })}
            </div>
          )}

          <div
            className="
              sticky top-[20%] mt-8 flex h-fit flex-col items-center
              border border-elements p-6 lg:col-[9_/_-1] lg:mt-0
            "
          >
            <h2 className="font-bold">{`$`}</h2>
            <p className="text-secondary">{`Total for ${sumOfQuantity} items`}</p>

            <ButtonCard className="mt-8 h-12 w-full md:mt-[48px]">
              Checkout
            </ButtonCard>
          </div>
        </div>
      </section>
    </main>
  );
};
