...
import { selectedInfoProduct, setProductInfo } from '../../features/productInfoSlice';
...
export const ProductDetails = () => {
  ...
  const [choosenProduct, setChoosenProduct] = useLocalStorage<TabAccessPhone | undefined>('product', undefined);

  const allProducts: TabAccessPhone[] = phones.concat(tablets, accessories);

  useEffect(() => setChoosenProduct(allProducts.find((item) => item.id === productId)), [productId]);
  ...
  const capacityLink = () => {
    if (!choosenCapasity) {
      return product.capacity.toLowerCase();
    }

    return choosenCapasity;
  }
              ...
              >
                  ...
                  <div className="details__product__itemHead">Avaliable colors</div>
                    <ul className="details__product__colorsVariants">
                      {product?.colorsAvailable.map((color: string, index) => {
                        return (
                          <Link
                            to={`/${paths[0]}/${product.namespaceId}-${capacityLink()}-${color}`}
                            key={index}
                  ...
                  <div className="details__product__itemHead">Select capacity</div>
                    <ul className="details__product__capacityVariants">
                      {product?.capacityAvailable.map((capacity: string, index) => {
                        return (
                          <Link
                            to={`/${paths[0]}/${product.namespaceId}-${capacity}-${colorLink()}`}
                            key={index} 
                ...
                <div className="details__product__priceContainer">
                  <div className='details__product__priceBlock'>
                    <div className="details__product__price">${product?.priceRegular}</div>
                    <div className="details__product__priceDiscount">${product?.priceDiscount}</div>
                    </div>
                    ...
                    <div className="details__description">
                      ...
                      <div className="details__description__model">{product?.screen}</div>
                      </div>
                      <div className="details__description__name">
                      <div className="details__description__item">Resolution</div>
                      <div className="details__description__model">{product?.resolution}</div>
                      </div>
                      <div className="details__description__name">
                      <div className="details__description__item">Processor</div>
                      <div className="details__description__model">{product?.processor}</div>
                      </div>
                      <div className="details__description__name">
                        <div className="details__description__item">RAM</div>
                        <div className="details__description__model">{product?.ram}</div>
                    </div>
                    </div>
                  </div>
                </div>
            ...
            details__section__descript--about'>
              ...
              <div className='details__section__descript'>
                ...
                <div className='details__section__item'>
                  ...
                  <div className='details__section__model'>{product?.resolution}</div>
                </div>
                <div className='details__section__item'>
                  <div className='details__section__name'>Processor</div>
                  <div className='details__section__model'>{product?.processor}</div>
                  </div>
                <div className='details__section__item'>
                ...
                <div className='details__section__item'>
                  ...
                  <div className='details__section__model'>{product?.capacity}</div>
                  </div>
                <div className='details__section__item'>
